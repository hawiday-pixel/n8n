...
function categorizeWorkflow(name) {
  ...
  if (lower.includes('xero') || lower.includes('invoice') || lower.includes('payable'))
    return 'finance';
  if (lower.includes('shopify') || lower.includes('cart') || lower.includes('product'))
    return 'shopify';
  if (lower.includes('job') || lower.includes('schedule') || lower.includes('servicem8'))
    return 'operations';
      ...
      const sanitizedNodes = workflow.nodes.map((node) => {
        ...
        if (sanitized.parameters) {
          ...
          const redacted = params
            ...
            .replace(
              /"(api[Kk]ey|apikey|token|secret|password|authorization)":\s*"[^"]+"/g,
              '"$1": "REDACTED"'
            )
            .replace(
